...
public class MastermindTester {
    ...
    private static final String testFilename = "C:\\Users\\shermanr\\Downloads\\mastermind_4p6c.txt";
    ...
    static class ExampleStudent extends StudentAlgorithm {
        ...
        public ExampleStudent() {
            super("RonenSherman"); // Put your name or GitHub username here
        }
        ...
        public int[] scoreCodewords(String codeword1, String codeword2) {
            ...
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    if (guessDigits[i] == secretDigits[j] && secretDigits[j] != guessDigits[j] && !list.contains(j) && secretDigits[i] != guessDigits[i] ) {
                        list.add(j);
                        w++;
                        break;
                    }
